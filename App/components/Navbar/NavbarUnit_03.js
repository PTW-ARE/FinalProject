...
const NavbarUnit_03 = ({ navigation = useNavigation() }) => {
  const [units, setUnits] = useState([]);
  const [selectedUnit, setSelectedUnit] = useState('U03');
    ...
    <StyledView className="bg-blue-800 p-4 rounded-b-3xl flex-row justify-between pt-8">
      ...
      <StyledView>
        {units
          ...
          .map((unit) => (
            <StyledText key={unit.UnitID} className="text-white text-2xl mr-2 font-bold flex-col justify-center items-center">
              {unit.UnitName}
            </StyledText>
          ))}
      </StyledView>

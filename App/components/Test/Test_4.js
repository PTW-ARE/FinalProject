    ...
    const handleNext = () => {
        ...
        navigation.dispatch(
            CommonActions.reset({
                index: 0,
                routes: [
                    { name: 'T05', params: { scores } } // ปรับตามหน้าที่จะไป
                ],
            })
        );
    };
            ...
            <StyledView className='bg-orange-200 mx-4 mt-1 mb-4 h-2/3 rounded-2xl'>
                ...
                <StyledView className='mt-5'>
                    <StyledTouchableOpacity onPress={() => handleChoice('Choice1', true)}
                        className={`my-4 ml-4 w-5/6 pl-4 py-2 shadow-md rounded-2xl ${selectedChoice === 'Choice1' ? 'bg-violet-700' : 'bg-blue-700'}`}>
                        {tests
                            .filter((test) => test.TestID === selectedTest)
                            .map((test) => (
                    ...
                    <StyledTouchableOpacity
                        ...
                        className={`my-4 ml-4 w-5/6 pl-4 py-2 shadow-md rounded-2xl ${selectedChoice === 'Choice2' ? 'bg-violet-700' : 'bg-blue-700'}`}>
                        {tests
                            .filter((test) => test.TestID === selectedTest)
                            .map((test) => (
                                <StyledText key={test.TestID} className='text-lg text-white '>
                    ...
                    <StyledTouchableOpacity
                        ...
                        className={`my-4 ml-4 w-5/6 pl-4 py-2 shadow-md rounded-2xl ${selectedChoice === 'Choice3' ? 'bg-violet-700' : 'bg-blue-700'}`}>
                        {tests
                            .filter((test) => test.TestID === selectedTest)
                            .map((test) => (
                                <StyledText key={test.TestID} className='text-lg text-white'>
